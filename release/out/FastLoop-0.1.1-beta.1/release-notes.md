# FastLoop 0.1.1-beta.1

## Downloads

- `FastLoop-Windows-x64-Setup.exe` is the primary installer for end users.
- `FastLoop-Windows-x64.zip` is the secondary portable package.

## What Is In This Prerelease

- CEP extension bundle for Premiere Pro and After Effects
- packaged Windows engine runtime so the standard release path does not require a manual Python install
- analyze, preview, export, and commit workflow in the FastLoop panel
- release manifest, checksums, and install documentation aligned with the GitHub Releases workflow

## Installation Path

1. Open the GitHub Releases page.
2. Download `FastLoop-Windows-x64-Setup.exe`.
3. Run the installer.
4. Open Premiere Pro or After Effects.
5. Launch the `FastLoop` panel and use it.

## Notes

- GitHub Releases is the main installation path.
- `FastLoop-Windows-x64-Setup.exe` remains the recommended asset.
- `FastLoop-Windows-x64.zip` remains the fallback asset.
- These assets are structured for future signing, but the current output remains an unsigned prerelease.

