...
import type { AnalysisResult, LoopCandidate, PanelBridge, ScoringMode } from "@fastloop/shared";
import { getBridge } from "./adobe.js";
import { createInitialState, type PanelState } from "./state.js";
...
export async function mountApp(root: HTMLElement, options?: MountOptions): Promise<void> {
  ...
  function syncQueue(status: PanelState["queue"][number]["status"]): void {
    const track = selectedTrack(state);
    if (!track) {
      return;
    }
